...
<div class="container" style="background: whitesmoke; margin-top: 40px;" >       
  <table class="table">
    <thead> <h2><%=member.getName() %> 님의 회원 정보</h2> 
        ...
        <td>아이디</td><td><%=member.getId() %></td>
         ...
         <td>비밀번호</td><td><button type="button" onclick="myFunction()">비밀번호 보기</button>
         			<button type="button" onclick="location.href='PassModify.ad?id=<%=member.getId()%>'">비밀변호 변경</button><br/>
	...
	<%if(pageNo !=null){%>
		<button type="button" class="btn btn-link" onclick="location.href='member_list.ad?pageNo=<%=pageNo%>'">리스트로</button>
	<% } else {%>
		<button type="button" class="btn btn-link" onclick="location.href='Membersearch.ad?pageNoN=<%=pageNoN%>&uid=<%=uid%>'">리스트</button>
	<%} %>
		<button type="button" class="btn btn-link" onclick="location.href='MemberModifyView.ad?id=<%=member.getId() %>'">회원 수정</button>
		<button type="button" class="btn btn-link" id="delete"> 회원 삭제</button>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<p><%=member.getName() %>님을 탈퇴하시겠습니까?</p>
					</div>
