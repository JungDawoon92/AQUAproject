...
public class MemberModifyAction implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(AdminCheck.adminCheck(request, forward) == true) {
			...
			int result = MemberBiz.update(member);
			...
			if (result == 0) {
				...
				out.println("<div id='myModal' class='modal'role='dialog'>  <div class='modal-content'> <div class='modal-header'>");
				out.println("<h4 class='modal-title'>알람</h4> </div>");
				out.println("<div class='modal-body'>");
				out.println("<p>회원이 정보가 수정되지 않았습니다.</p> </div>");
				...
				out.println("<h4 class='modal-title'>알람</h4> </div>");
				out.println("<div class='modal-body'>");
				out.println("<p>회원이 정보가 수정되었습니다.</p> </div>");
				out.println("<div class='modal-footer'>");
				out.println("<button type='button' class='btn btn-default' data-dismiss='modal' onClick='close_pop();'>확인</button>");
				out.println("</div></div></div>");
				out.println("<script>");
